...
def pr2_mover(object_list):
    ...
    for detected_object in object_list:
        ...
        #lables.append(detected_object.label)
        points_arr = ros_to_pcl(detected_object.cloud).to_array()
        centroid = np.mean(points_arr, axis = 0)[:3]
        #centroids.append(np.asscalar(centroid))
        lables[detected_object.label] = [np.asscalar(centroid[0]),np.asscalar(centroid[1]),np.asscalar(centroid[2])]
   
    for dropbox in dropbox_param:
    	g = dropbox['group']
        dropbox_name[g] = dropbox['name']
        dropbox_position[g]  = dropbox['position'] 
    ...
    for get_object in object_list_param:
        ...
        if n not in lables:
        	print "object is not found: ", n
        	continue
        print "object is found: ", n	
        ...
        place_pose = Pose()
        place_pose.position.x = dropbox_position[g][0]
        place_pose.position.y = dropbox_position[g][1]
        place_pose.position.z = dropbox_position[g][2]
    ...
    yaml_filename = "output_test" + str(test_scene_num.data) + ".yaml"
